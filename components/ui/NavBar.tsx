    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-white/85 backdrop-blur-xl dark:border-white/5 dark:bg-slate-950/85">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8" aria-label="全局导航">
        <Link href="/" className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-950 text-sm font-bold text-white dark:bg-amber-400 dark:text-slate-950">
            围
          </span>
        ...
        <form onSubmit={search} className="mx-6 hidden max-w-md flex-1 sm:block">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-100 px-4 py-2 text-sm text-slate-900 outline-none transition focus:border-pink-400 focus:ring-2 focus:ring-pink-200 dark:border-white/10 dark:bg-white/5 dark:text-white"
          />
        </form>
        ...
        <div className="hidden items-center gap-7 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`text-sm font-semibold transition ${active(link.href) ? "text-pink-500" : "text-slate-500 hover:text-slate-950 dark:text-slate-400 dark:hover:text-white"}`}
            >
              {link.label}
            </Link>
          ))}
          ...
          {session && (
            <Link href="/messages" className={`relative text-sm font-semibold ${active("/messages") ? "text-pink-500" : "text-slate-500 hover:text-slate-950"}`}>
              消息
              {unreadCount > 0 && (
          ...
          {session ? (
            <div className="relative">
              <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex h-9 w-9 items-center justify-center rounded-full bg-pink-500 text-sm font-bold text-white">
                ...
                <>
                  ...
                  <div className="absolute right-0 z-20 mt-2 w-48 rounded-xl bg-white py-1 shadow-xl ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-white/10">
                    <div className="border-b border-slate-100 px-4 py-2 dark:border-white/10">
                      ...
                      {isAdmin && <p className="text-xs text-pink-500">管理员</p>}
                    </div>
                    <MenuLink href={`/profile/${session.user?.name}`} onClick={() => setUserMenuOpen(false)}>我的主页</MenuLink>
                    {isAdmin && <MenuLink href="/admin" onClick={() => setUserMenuOpen(false)}>管理面板</MenuLink>}
                    <MenuLink href="/premium" onClick={() => setUserMenuOpen(false)}>会员</MenuLink>
            ...
            <Link href="/login" className="text-sm font-semibold text-slate-900 hover:text-pink-500 dark:text-white">
              登录
            </Link>
          )}
        </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-white p-6 dark:bg-slate-900 lg:hidden">
          <div className="mb-6 flex items-center justify-between">
            <Link href="/" className="text-lg font-bold" onClick={() => setMobileOpen(false)}>围炉</Link>
          ...
          <div className="space-y-2">
            ...
            {session && <Link href="/messages" onClick={() => setMobileOpen(false)} className="block rounded-xl px-4 py-3 font-semibold">消息</Link>}
            {isAdmin && <Link href="/admin" onClick={() => setMobileOpen(false)} className="block rounded-xl px-4 py-3 font-semibold">管理面板</Link>}
          </div>
        </div>
      )}
    </header>
