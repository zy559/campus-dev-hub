...
export default function NewConversation({ onClose }: { onClose: () => void }) {
  ...
  function avatarColor(name: string) {
    const colors = ["bg-teal-500", "bg-sky-500", "bg-emerald-500", "bg-violet-500", "bg-cyan-500", "bg-blue-500"];
    let hash = 0;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center px-3 pb-20 pt-6 sm:items-start sm:px-4 sm:pb-6 sm:pt-16">
      <div className="fixed inset-0 bg-black/30 backdrop-blur-sm" onClick={onClose} />
      <div className="relative flex max-h-[calc(100vh-6.5rem)] w-full max-w-2xl animate-scale-in flex-col overflow-hidden rounded-[1.75rem] bg-white shadow-2xl ring-1 ring-slate-200 sm:max-h-[calc(100vh-5rem)]">
              ...
              <div className="mt-4 max-h-80 overflow-y-auto">
                {loading ? (
                  ...
                  <div className="rounded-2xl bg-slate-50 px-4 py-8 text-center">
                    <p className="text-sm font-bold text-slate-700">搜索用户名后即可发起聊天</p>
                    <p className="mt-2 text-xs leading-5 text-slate-500">手机端可以在这个弹层内上下滑动，搜索同学区域不会被底部导航挡住。</p>
                  </div>
                )}
              </div>
            </div>
