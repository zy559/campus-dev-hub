...
export const PostSchema = z.object({
  ...
  tagIds: z.array(z.string()).max(8, "最多选择 8 个标签").default([]),
  tagNames: z.array(z.string()).max(8, "最多选择 8 个标签").optional(),
  boardId: z.string().optional(),
});

export const CommentSchema = z.object({
  content: z.string().min(1, "评论不能为空").max(2000, "评论最多 2000 个字符"),
});

export const MessageSchema = z.object({
  content: z.string().min(1, "消息不能为空").max(5000, "消息最多 5000 个字符"),
});
