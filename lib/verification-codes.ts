...
export async function setCode(email: string, code: string): Promise<void> {
  ...
  console.log("[setCode] storing for:", normalizedEmail, "code:", code);
  try {
    ...
    console.log("[setCode] stored successfully");
  } catch (err) {
    console.error("[setCode] DB error:", err);
    throw err;
  }
}
...
export async function verifyCode(email: string, code: string): Promise<boolean> {
  const normalizedEmail = email.toLowerCase();
  console.log("[verifyCode] looking for:", normalizedEmail);
  try {
    const entry = await db.verificationCode.findUnique({
      where: { email: normalizedEmail },
    });
    console.log("[verifyCode] found entry:", entry ? "yes" : "NO");
    if (!entry) return false;
    if (entry.expiresAt < new Date()) {
      console.log("[verifyCode] expired at:", entry.expiresAt.toISOString());
      await db.verificationCode.delete({ where: { email: normalizedEmail } });
      return false;
    }
    if (entry.code !== code) {
      console.log("[verifyCode] code mismatch — stored:", entry.code, "given:", code);
      return false;
    }
    await db.verificationCode.delete({ where: { email: normalizedEmail } });
    return true;
  } catch (err) {
    console.error("[verifyCode] DB error:", err);
    return false;
  }
}
